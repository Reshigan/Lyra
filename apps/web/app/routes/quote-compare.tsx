...
const LABELS: Record<string, Record<string, string>> = {
  en: {
    ...
    approvalBody: "Policy {policy} must approve this selection before it stands.",
  ...
  ar: {
    ...
    approvalBody: "يجب أن توافق السياسة {policy} على هذا الاختيار قبل اعتماده.",
    ...
    <div className="flex flex-col gap-6">
      ...
      {loaded.offers.length ? (
        <section aria-labelledby="offers-heading" className="flex flex-col gap-3">
          <h2 id="offers-heading" className="font-display text-16 text-text">
            {L("offersTitle")}
          </h2>
  ...
  return (
    <section aria-labelledby={id} className="flex flex-col gap-3">
      <div className="flex flex-col gap-1">
        <h2 id={id} className="font-display text-16 text-text">
          {title}
        </h2>
