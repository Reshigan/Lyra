 ...
 */
function shapeOf(cols: Columns, mode: "create" | "update"): z.ZodTypeAny {
...
export function crudRouter(r: Resource): Hono<App> {
  ...
  const createShape = shapeOf(cols, "create");
  const updateShape = shapeOf(cols, "update");
  const idCol = cols.id;
  ...
  const load = async (ctx: Ctx, rowId: string, withDeleted = false): Promise<Record<string, unknown>> => {
    ...
    return assertTenant(ctx, rows[0] as never, r.path) as Record<string, unknown>;
  };
  ...
  if (r.perms.create) {
    ...
    app.post("/", async (c) => {
      ...
      return withIdempotency(ctx, c.req.header("idempotency-key"), `POST ${c.req.path}`, input, async () => {
        let values: Record<string, unknown> = { ...input, ...(r.fixed?.(ctx, "create") ?? {}) };
        ...
        await audit(ctx, { action: `${auditName}.create`, subjectRef: rowId, after: values });
  ...
  if (r.perms.remove && !r.immutable) {
    ...
    app.delete("/:id", async (c) => {
      ...
      await audit(ctx, { action: `${auditName}.delete`, subjectRef: rowId, before: hydrate(before) });
    ...
    if (hasSoftDelete) {
      app.post("/:id/restore", async (c) => {
        ...
        await audit(ctx, { action: `${auditName}.restore`, subjectRef: rowId, before: hydrate(before) });
