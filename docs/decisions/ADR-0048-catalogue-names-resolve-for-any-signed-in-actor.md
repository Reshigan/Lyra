# ADR-0048: Provider and product names resolve for any signed-in actor
...
it needs an ADR of its own. This is that ADR, for `providers` and `products`.
...
policy, and the mobile equivalents.
...
`providers` and `products` join `users` and `teams` in `DIRECTORY`. Every other
property of the endpoint is unchanged and is what makes this safe:
...
- **Masking holds** — neither table declares PII today; if one ever does, the
...
providers as `prv` and products as `prd`, but every seeded tenant carries `pv`
and `pr`. Those rows resolved to nothing regardless of permission. They join
`usr` in the `ALIASES` table, the same remedy ADR-0046 applied to users.
...
The seam rule from ADR-0046 stands and now covers four resources. The line it
